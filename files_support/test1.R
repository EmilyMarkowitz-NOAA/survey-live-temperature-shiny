    ...
    if (nrow(dat0) == 0) {
      ...
      if (show_planned_stations) {
        temp <- data.frame(matrix(data = NA,
                                  ncol = ncol(dat),
                                  nrow = nrow(dat_surveys)))    
        names(temp) <- names(dat)
        temp <- temp %>% 
          dplyr::mutate(
            SRVY = dat_surveys$SRVY,
            stratum = 0,
            station = "0",
            var0 = NA,
            region_long = dat_surveys$region_long, 
            survey_dates  = dat_surveys$survey_dates, 
            date = (min(as.Date(dat$date), na.rm = TRUE)-1),
            vessel_name = dat_surveys$vessel_name,
            vessel_shape = dat_surveys$vessel_shape)
