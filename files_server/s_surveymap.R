find_coldpool <- function(input) {
  ...
  if (input$plot_unit == "bottom_temperature_c") {
    if ("EBS" %in% input$survey & "NBS" %in% input$survey & # is this data requested?
      ...
      df2 <- coldpool::nbs_ebs_bottom_temperature
      r0 <- "nbs_ebs_bottom_temperature"
    } else if ("EBS" %in% input$survey & # is this data requested?
               "EBS" %in% temp$SRVY ) { # is this data in the dataset?
      df2 <- coldpool::ebs_bottom_temperature
      r0 <- "ebs_bottom_temperature"
    } else if ("NBS" %in% input$survey & # is this data requested?
               "NBS" %in% temp$SRVY ) { # is this data in the dataset?
      df2 <- coldpool::nbs_bottom_temperature
      r0 <- "nbs_bottom_temperature"
    }
  }
  ...
  if (input$plot_unit == "surface_temperature_c") {
    if ("EBS" %in% input$survey & "NBS" %in% input$survey & # is this data requested?
        "EBS" %in% temp$SRVY & "NBS" %in% temp$SRVY) { # is this data in the dataset?
      df2 <- coldpool::nbs_ebs_surface_temperature
      r0 <- "nbs_ebs_surface_temperature"
    } else if ("EBS" %in% input$survey & # is this data requested?
               "EBS" %in% temp$SRVY ) { # is this data in the dataset?
      df2 <- coldpool::ebs_surface_temperature
      r0 <- "ebs_surface_temperature"
    } else if ("NBS" %in% input$survey & # is this data requested?
               "NBS" %in% temp$SRVY ) { # is this data in the dataset?
      df2 <- coldpool::nbs_surface_temperature
      r0 <- "nbs_surface_temperature"
    }
  }
  ...
  output$survey_leaflet <- renderLeaflet({
    ...
    a <- 
      ...
      addPolygons(
        ...
        color          = "#444444",
        opacity        = 1,
        # fillColor      = ~pal(survey_definition_id), # MAY NEED TO FIX THIS
        # fillOpacity  = 0.2,
        # smoothFactor = 0.5,
